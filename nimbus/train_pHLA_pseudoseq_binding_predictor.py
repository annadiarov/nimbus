...
from nimbus.predictors import pHLAPseudoseqBindingPredictor
...
from nimbus.data_processing import pHLAPseudoseqDataset
...
def parse_args():
    ...
    parser.add_argument('--hla_fp_dim',
                        dest='hla_fp_dim',
                        type=int, default=80,
                        help='Dimension of HLA pseudosequence embedding')
    ...
    parser.add_argument('--hla_n_fp',
                        ...
                        type=int,
                        default=34,
                        help='Length of HLA pseudosequence')
...
def load_model(config):
    if config['pretrained_filename'] != '' and os.path.isfile(config['pretrained_filename']):
        ...
        model = pHLAPseudoseqBindingPredictor.load_from_checkpoint(config['pretrained_filename'], **config)
    else:
        model = pHLAPseudoseqBindingPredictor(**config)
    return model
...
if __name__ == '__main__':
    ...
    if config['train']:
        ...
        val_dataset = pHLAPseudoseqDataset(
    ...
    if config['predict']:
        ...
        test_dataset = pHLAPseudoseqDataset(
