...
class BidirectionalCrossAttentionBlock(nn.Module):
    def __init__(
        ...
        self.talking_heads = nn.Conv2d(heads, heads, 1,
                                       bias=False) if talking_heads else nn.Identity()
        self.context_talking_heads = nn.Conv2d(heads, heads, 1,
                                               bias=False) if talking_heads else nn.Identity()
    ...
    def forward(
        ...
        b, i, j, h, device = x.shape[0], x.shape[-2], context.shape[
            -2], self.heads, x.device
        ...
        context_qk, context_v = self.context_to_qk(context), self.context_to_v(
            context)
        ...
        out, context_out = map(lambda t: rearrange(t, 'b h n d -> b n (h d)'),
                               (out, context_out))
